...
def compile_nmdr_extracts(
    ...
    """Saves per-period extracted DataFrames to parquet.
    ...
    Args:
        ...
        output_path (Path): Path where the compiled extracts are saved.
    """
    current_run.log_info("Compiling nmdr extracts..")
    output_path.mkdir(parents=True, exist_ok=True)

    for period in extract_periods:
        df = data_by_period.get(period)
        if df is None or df.is_empty():
            current_run.log_info(f"No data found for period {period}. Skipping extract.")
            continue
        save_to_parquet(data=df, filename=output_path / f"nmdr_extract_{period}.parquet")
        current_run.log_info(f"NMDR extract for period {period} saved at {output_path / f'nmdr_extract_{period}.parquet'}")
