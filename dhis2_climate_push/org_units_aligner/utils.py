...
def log_message(
    ...
    level : str, optional
        The logging level ('info', 'debug', 'warning', 'error'), by default 'info'.
    ...
    if level not in LOGGING_LEVELS:
        raise exception_class(f"Invalid logging level: {level}. Must be one of {LOGGING_LEVELS}.")

    full_message = message + (f" | Details: {error_details}" if error_details else "")
