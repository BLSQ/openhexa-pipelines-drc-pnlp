...
class Era5:
    def __init__(self, cache_dir: str = None):
        ...
        self.cds_api_url = "https://cds.climate.copernicus.eu/api"
    ...
    def init_cdsapi(self):
        ...
        # logger.debug(f"key: {connection.key}")
        self.api = cdsapi.Client(url=self.cds_api_url, key=connection.key)
    ...
    def download(
        ...
        if "valid_time" in ds.variables:
            ...
            time_var = "time"
        if not ds[time_var].values.max() >= pd.to_datetime(datetime(year, month, n_days)):
    ...
    def _build_request(
        self, variable: str, bounds: Tuple[float], start: datetime, end: datetime, hours: List[str]
    ) -> dict:
        """
    ...
    def _is_valid_dataset(self, ds_filename: str, start: datetime, end: datetime, hours: List[str]) -> bool:
        ...
        valid_range = [
            [
                pd.Timestamp(
                    datetime.strptime(f"{(start + timedelta(days=d)).strftime('%Y-%m-%d')}T{h}:00", "%Y-%m-%dT%H:%M:%S")
                )
        ...
        with xr.open_dataset(ds_filename) as ds:
            ...
            ds_filtered = ds.sel(
                {time_var: ((ds[time_var] >= valid_range.min()) & (ds[time_var] <= valid_range.max()))}
            )

        if len(ds_filtered[time_var].values) == len(valid_range):
