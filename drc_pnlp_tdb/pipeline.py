...
@pipeline(code="drc-pnlp-tdb", name="DRC PNLP TdB", timeout=28800)  # (8 * 60 * 60))
@parameter(
    "get_year",
    name="Year",
...
def pnlp_extract_process(get_year, get_download, get_run_notebooks, get_upload, *args, **kwargs):
    ...
    if get_download:
        ...
        routine_download_complete = extract_dhis_data(RAW_DATA_DIR, get_year, "routine")

        all_cause_mortality_download_complete = extract_dhis_data(
            RAW_DATA_DIR, get_year, "acm", routine_download_complete
        )
...
def extract_dhis_data(output_dir, year, mode, *args, **kwargs):
    ...
    for period in extract_periods:
        ...
        dhis2_download_analytics(output_dir, period, mode)
